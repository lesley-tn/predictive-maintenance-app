  ...
  <div class="px-12 py-10">
    ...
    <div class="flex gap-4 py-8">
      ...
      <input
        ...
        dense
        placeholder="Search"
        class="py-2 px-4 min-h-full rounded-xl border-2 border-lnf-navy/10 flex-grow text-body"
        v-model="search"
      />
      ...
      <div>
        <q-btn-toggle
          v-model="toggle"
          spread
          class="clients-buildings-toggle text-body rounded-xl h-full"
          unelevated
  ...
  setup() {
    ...
    watch(search, (newSearch) => {
      clientsToShow.value = clients.value.filter((client) => {
        if (
          newSearch.toLowerCase().split(' ').length > 1 &&
          newSearch.toLowerCase().split(' ')[1] != null
        ) {
          if (newSearch.toLowerCase().includes('building')) {
            // Specific case when "building" is included in the search

            return client.Clients_id.toString()
              .toLowerCase()
              .includes(
                get_clients_id_from_building_id(
                  parseInt(newSearch.toLowerCase().split(' ')[1]).toString()
                )
              );
          } else if (newSearch.toLowerCase().includes('client')) {
            return client.Clients_id.toString()
              .toLowerCase()
              .includes(newSearch.toLowerCase().split(' ')[1]);
          }
        } else {
          return (
            client.Clients_name.toLowerCase().includes(
              newSearch.toLowerCase()
            ) ||
            client.Clients_id.toString()
              .toLowerCase()
