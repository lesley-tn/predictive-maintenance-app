          ...
          <q-tab-panels v-model="selectedTab">
            ...
            <q-tab-panel name="buildings">
              <div>
                ...
                <q-input label="Description" v-model="buildingsData.description" />
              </div>
            </q-tab-panel>
          </q-tab-panels>
          <q-card-actions align="right">
            <q-btn label="Cancel" color="primary" @click="closeDataInputDialog" />
            <q-btn label="Save" color="primary" @click="saveData" v-on:click="$emit('refresh-event')" />
          </q-card-actions>
        </q-card>
      </q-dialog>
    </div>
    ...
    setup() {
      ...
      const saveData = () => {
        if (selectedTab.value === 'clients') {
          ...
          supabase.from('Clients').upsert([dataToInsert]).then((response) => {
            if (response.error) {
